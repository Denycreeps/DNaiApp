...
사실상 99.9% 정도를 Claude와 Gemini가 만든 Ai가 만든 Ai 이미지 만들기를 돕는 어플리케이션.
...
다른 앱이나 프로그램에서 쓰는 방식(Danbooru 만으로 api없이 검색)으로 충분히 써봄직한 방법을 찾으면 그땐 뺄 예정

근데 지금 방식도 나름 직접 써보니 속도도 빠르고 꽤 괜찮음
...
ex) skirt가 포함된 모든 프롬프트를 검색
같은 기능은 아직 안만들었음... 사용량 거의다써서...
...
- *skirt = 뒤가 skirt인 프롬프트를 제거
- skirt* = 앞이 skirt인 프롬프트를 제거

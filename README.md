...
검색할때 `*`을 붙인 곳에 따라 범위가 바뀌고 (별 붙일때 띄어쓰기 안해도됨), 윗칸 아래칸 모두 사용 가능함.
...
기타 탭에 있는 프리셋은 이렇게 나오고 해당하는 항목을 누르면 펼칠 수 있음. 앞에 3개는 복 복사하거나 적용할때 그 프롬프트만 나오지만 기타 탭에 있는 프리셋을 불러올땐 내가 원하는 것만 가져올 수 있게 해뒀음. 

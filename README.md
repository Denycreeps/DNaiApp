...
<br />
<img width="383" height="72" alt="image" src="https://github.com/user-attachments/assets/b4461bc0-1493-43aa-b6c6-b98476b5a164" />

하나 다른게 있다면 이미지를 선택했을 때 오른쪽 위에 `동그라미 i` 버튼이 생김. 이건 이미지를 1개만 골랐을때랑 여러개 골랐을때가 다른데
...
추가로 그냥 이미지를 꾹 누르지않고 그냥 터치하면 이미지를 확대해서 볼 수 있는데
...
이 기능을 안쓸것이다 + 거슬려서 보기싫다 하는 사람들은

설정에 ON/OFF가 있으니 꺼두는것도 가능
...
이미지를 꾹 눌러서 나오는 창 중에 '이미지 수정하기(i2i)' 를 누르면 자동으로 이 `i2i탭`으로 오면서 이미지가 놓여짐
...
<img width="108" height="131" alt="image" src="https://github.com/user-attachments/assets/11848738-7bd4-42d3-91e6-8dba9331e77d" />

오른쪽 아래에 이런 버튼이 있는데 이게 무엇이냐면

<br />
<img width="381" height="676" alt="image" src="https://github.com/user-attachments/assets/5f09fd32-e462-433f-b634-79152b204202" />

이미지를 i2i로 보내거나 인페인트/모자이크/업스케일 을 실행하면 여기에 저장됨

원래는 히스토리에 쌓였는데 지저분하기도 하고 불편한점이 좀 있어서 새로 만듬

<br/>
<img width="365" height="340" alt="image" src="https://github.com/user-attachments/assets/e5f82ecf-f005-427d-bd2c-6478d7485d4c" />

예시로 인페인트를 하게되면(입 부분에 마스킹) 이렇게 히스토리에 계속 쌓이며(화면 오른쪽)

인페인트 결과 이미지로 전환됨. 이 `인페인트 자동 전환`이랑 `인페인트 후 마스킹 유지`는 설정에서 ON/OFF 가능.

<br />

또한 이 `i2i 히스토리 핸들` 자체를 설정에서 OFF로 하게되면 `i2i탭`에서 진행한 작업들은
...
현재는 4가지 탭으로 나눠놨고 각 탭에 있는 기능에 대해 설명함
...
*NovelAi 권장 순서 무시*
 - 대강 위 스크린샷이 NovelAi 권장 프롬프트 순서이고 `랜덤 프롬프트`를 누르면 이 기준을 토대로 기본 정렬을 1번 함.
...
*연속 생성 딜레이*
 - 이미지 생성시 연속으로 생성하는걸 켰을때 중간에 딜레이를 몇초로 할지 정함. 0초부터 5초까지 0.1단위로 지정 가능

<br />

*프롬프트 입력 폰트 크기*
 - 프롬프트탭, i2i탭, 캐릭터탭에서 프롬프트를 작성할때 따로 창이 뜨는 애들의 폰트를 조절 가능함
...
*갤러리 모드 비활성화*
 - 갤러리 모드가 쓰기 싫다, 보기 싫다 하는 사람들은 이걸 ON

<br />

*히스토리 이미지 슬라이드*
 - 히스토리탭에 떠있는 큰 이미지 좌/우에 화살표가 생김. 잘 안쓰는 기능이라 기본 OFF로 해뒀는데

<img width="377" height="326" alt="image" src="https://github.com/user-attachments/assets/44ec67d8-d7b0-42c2-a517-a95bcc50cdeb" />

ON이면 이런식으로 나옴. 만든게 아까워서 놔둔 기능

<br />

*i2i용 히스토리 비활성화*
 - 히스토리탭에 있는 히스토리 핸들을 비활성화 하는 기능.

이게 OFF면 i2i탭에서 작업한 결과물들은 모두 히스토리로 감

<br />

*인페인트 결과 자동 전환*
*인페인트 시 마스킹 자동 해제*
 - 위에 i2i용 히스토리 비활성화가 OFF여야 나오는 기능. i2i탭에서 설명한 그대로임

<br />

*가중치 색상 표시*
 - NovelAi 공식 홈페이지에 있는 그 기능.

<img width="298" height="46" alt="image" src="https://github.com/user-attachments/assets/a630e3f0-a35a-4638-b02c-af444d6f20e8" />

이런식으로 표시되는걸 최대한 따라해봤음. 있고 없고 가시성 차이가 좀 있는거 같음

<br />

*e621 프롬프트 확장*
 - 아는 사람은 아는 e621(퍼리 위주의 아카이브) 태그를 프롬프트 미리보기에 표시되도록 해줌.

<img width="290" height="170" alt="image" src="https://github.com/user-attachments/assets/6fc78eb1-20f0-4099-980d-ffe7bb8f5522" />

이런식으로 파란색 표시가 되서 나옴. 당연하지만 gelbooru, danbooru에는 없는 태그라 검색기능에선 사용 안되고

NovelAi한테 시킬때만 사용할 수 있음. 인식이 잘 되는지는 검증 필요.

<br />

*좌우 스와이프*
 - 이걸 켜면 드래그해서 각 탭을 이동할 수 있는데 `i2i탭`에서는 엄청 불편해서 기본적으로 꺼뒀음. 어차피 쓸사람도 없을것 같은 기능...
...
*탭 표시 설정*
 - 버튼을 터치해서 회색으로 만들면 그 탭이 안보임. 

단, `히스토리탭`을 꺼도 이미지는 저장되고 `i2i탭`을 꺼도 `i2i탭`으로 보내기를 하면 탭이 켜지면서 이미지가 올라감
...
<img width="373" height="729" alt="image" src="https://github.com/user-attachments/assets/96204b9b-b2e7-4e25-aa13-5b2e16fc60a3" />

*저장 폴더 (SAF)
 - 이건 맨 위에 초기설정쪽에서 설명했으니 패스.

아래에 `앱 폴더의 기존 이미지를 SAF로 옮기기`는 기존 사용자용 버튼.

<br />

*파일 이름* 
 - 말 그대로 파일의 저장 형식을 정할 수 있음

파일 이름은 연도=`{yy}` 월=`{mm}` 일=`{dd}` 시간=`{time}` 이 규칙이고 나머지는 적는 그대로 추가되서 알기 편할거임

번호=`{count}`는 바로 아래에 있는 `현재 생성된 이미지` 갯수를 따라가는데 시간까지 겹칠껄 대비한거지만

같은 1초안에 2번 생성될리 없으니 그냥 있기만 한 기능...

<br />

*현재 생성된 이미지* 
 - 현재 앱을 켠 상태에서 이미지가 몇개 저장됐는지 알려주는 기능. NAIA에 있어서 넣어본거긴 한데...

<br >

*설정 백업*
 - 현재 정해진 모든 설정을 저장(프롬프트, 세팅, 캐릭터, Api 등등 모든 것) 또는 저장된 설정을 불러올 수 있음.

폴더에 파일 형식으로 저장하거나 `공유`기능으로 카톡이나 구글 드라이브같은곳에 보낼수도 있음.

어지간하면 대부분 상황에서 설정이 초기화되는걸 막아놨는데 혹시모르니 가~끔 해두면 좋을듯

---

### API

<img width="371" height="397" alt="image" src="https://github.com/user-attachments/assets/8042de3d-f6a0-4bd3-8c15-8e2ba00f8796" />

여기도 초기설정에서 다 설명되어있으니 넘어감

---

### 기타

<img width="377" height="242" alt="image" src="https://github.com/user-attachments/assets/9a87b7ae-c830-4e97-996d-5469329bf2d0" />

github에 새로운 버젼이 나왔는지 앱을 처음 켤때 검사하게 하거나

수동으로 있는지 검사할때 사용하는 기능들임.
...
<br />
<img width="1440" height="1595" alt="KakaoTalk_20260706_195141948" src="https://github.com/user-attachments/assets/0db26c8c-1583-4e0b-bf65-8cdec5599749" />

이런식으로 업데이트 내역 앞쪽이랑(중요한건 앞에 둠) 다운받을지가 나오고

<br />
<img width="1440" height="776" alt="KakaoTalk_20260706_195141948_01" src="https://github.com/user-attachments/assets/b0df3469-2c2c-40ad-a4af-4ce47bb524b6" />

자동으로 apk파일을 받아서 업데이트할지 창이 뜸.

단, 구글 플레이스토어에 등록된 앱이 아니라서 보안 어쩌구를 풀고 업데이트 시작해야됨
...
3. 왜 앱 설치할때 or 업데이트 할때마다 보안 어쩌구 뜨는거임?
<br />

플레이스토어에 등록된 어플이 아니면 무조건 뜨는 경고창이라고 함.

정 불안하면 매번 앱 검사 버튼 눌러서 검사해도되고

업데이트 전에 https://www.virustotal.com/ 에서 앱 검사 돌려도 괜찮음.

보안에 민감한게 딱히 나쁜짓도 아니니까
...
맨위에도 써둔거같지만 앱에 대해서 건의사항이나 버그 신고같은건 https://arca.live/b/aiart/171383321 에서 부탁함
